...
function getCalculatorLabel(type: string): string {
  switch (type) {
    case 'calories': return '🔥 حاسبة السعرات';
    case 'bmi': return '📊 حاسبة BMI';
    case 'water': return '💧 حاسبة الماء';
    case 'ideal': return '⚖️ الوزن المثالي';
    default: return '';
  }
}
...
export function ArticleDetailView({
  ...
  article: Article;
  relatedArticles: Omit<Article, 'content'>[];
}) {
  ...
  const { initialContent, remainingContent } = useMemo(() => {
    ...
    return {
      initialContent: lines.slice(0, mid).join('\n'),
  ...
  const copyLink = () => { navigator.clipboard.writeText(articleUrl); toast.success('تم نسخ الرابط ✅'); };
  ...
  const shareButtons = [
    ...
    { fn: shareTwitter, label: 'X / تويتر', icon: <svg className="w-[18px] h-[18px]" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> },
    { fn: shareFacebook, label: 'فيسبوك', icon: <svg className="w-[18px] h-[18px]" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> },
    { fn: copyLink, label: 'نسخ الرابط', icon: <span className="text-sm">🔗</span> },
  ];
    ...
    <article className="page-transition">
      ...
      <nav className="mx-auto max-w-3xl px-4 pt-4 sm:px-6" aria-label="التنقل">
        <ol className="flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
          <li><Link href="/" className="link-hover hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">الرئيسية</Link></li>
          <li className="text-border/60">/</li>
          <li><Link href="/articles" className="link-hover hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">المقالات</Link></li>
          <li className="text-border/60">/</li>
      ...
      <div className="relative aspect-[21/9] sm:aspect-[21/7] overflow-hidden bg-gradient-to-br from-emerald-100 via-emerald-50 to-teal-50 dark:from-emerald-900/30 dark:via-emerald-950/20 dark:to-teal-950/20">
        {article.coverImage ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-8xl sm:text-9xl opacity-20">{article.calculatorType === 'calories' ? '🔥' : article.calculatorType === 'bmi' ? '📊' : article.calculatorType === 'water' ? '💧' : article.calculatorType === 'ideal' ? '⚖️' : '📋'}</div>
          </div>
        )}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            ...
            {article.calculatorType && (
              <span className="px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 text-xs font-medium border border-emerald-200/50 dark:border-emerald-800/30">
                {getCalculatorLabel(article.calculatorType)}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="mx-auto max-w-3xl px-4 py-5 sm:px-6">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {shareButtons.map((btn) => (
            <button key={btn.label} onClick={btn.fn} className="btn-press p-2.5 rounded-xl text-muted-foreground hover:bg-muted hover:text-foreground transition-all duration-200" title={btn.label}>
              {btn.icon}
            </button>
          ))}
        </div>
            ...
            {article.calculatorType && (
              <div className="mt-12 rounded-2xl bg-gradient-to-l from-emerald-600 to-emerald-500 dark:from-emerald-700 dark:to-emerald-600 p-8 sm:p-10 text-center shadow-xl shadow-emerald-600/15 dark:shadow-emerald-900/30 animate-[slideUp_0.5s_ease-out]">
                <div className="text-4xl mb-4">
                  {article.calculatorType === 'calories' ? '🔥' : article.calculatorType === 'bmi' ? '📊' : article.calculatorType === 'water' ? '💧' : '⚖️'}
                </div>
                <h2 className="text-xl sm:text-2xl font-bold text-white mb-3">
                  {article.calculatorType === 'calories' ? '🔥 احسب احتياجك من السعرات الآن' : article.calculatorType === 'bmi' ? '📊 احسب مؤشر كتلة جسمك الآن' : article.calculatorType === 'water' ? '💧 احسب كمية الماء التي تحتاجها' : '⚖️ اكتشف وزنك المثالي'}
                </h2>
                <p className="text-emerald-100 mb-7 text-sm sm:text-base">اكتشف احتياجك اليومي واحصل على خطة مخصصة مجانية</p>
                <Link href="/?view=calculators" className="btn-press inline-flex items-center gap-2.5 rounded-xl bg-white dark:bg-emerald-50 px-8 py-3.5 font-bold text-emerald-700 dark:text-emerald-800 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5">
                  ابدأ الحساب الآن
              ...
              <div className="mt-14 border-t border-border/60 pt-10">
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {relatedArticles.map((a) => (
                    <Link key={a.id} href={`/article/${a.slug}`} className="group btn-press text-right rounded-2xl border border-border/50 p-4 transition-all duration-300 hover:shadow-lg hover:shadow-emerald-900/5 dark:hover:shadow-emerald-900/15 hover:border-emerald-200/80 dark:hover:border-emerald-800/50 hover:-translate-y-1">
